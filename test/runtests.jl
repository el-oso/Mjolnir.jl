...
@testset "Mjolnir — Stage 4e: containers.Map, strings, method calls" begin
    ...
    @testset "builtin mappings exercised (coverage)" begin
        cases = [
            "y = fieldnames(s);" => "collect(string.(keys",
            "y = isfield(s, 'a');" => "haskey(s, Symbol",
            "y = rmfield(s, 'a');" => "structdiff",
            "warning('m');" => "@warn",
            "y = keys(m);" => "collect(keys(",
            "y = values(m);" => "collect(values(",
            "y = isKey(m, k);" => "haskey(m, k)",
            "remove(m, k);" => "delete!(m, k)",
            "y = eye(3);" => "LinearAlgebra.I",
            "y = inv(A);" => "inv(A)",
            "y = norm(v);" => "norm(v)",
            "y = dot(a, b);" => "dot(a, b)",
            "y = cross(a, b);" => "cross(a, b)",
            "y = trace(A);" => "tr(A)",
            "y = det(A);" => "det(A)",
            "y = eig(A);" => "eigvals(A)",
            "y = reshape(A, 2, 3);" => "reshape(A, 2, 3)",
            "y = fliplr(A);" => "reverse(A, dims = 2)",
            "y = flipud(A);" => "reverse(A, dims = 1)",
            "y = mean(v);" => "mean(v)",
            "y = std(v);" => "std(v)",
            "y = var(v);" => "var(v)",
            "y = gcd(a, b);" => "gcd.(a, b)",
            "y = lcm(a, b);" => "lcm.(a, b)",
            "y = nchoosek(n, k);" => "binomial(n, k)",
            "y = kron(A, B);" => "kron(A, B)",
            "y = ismember(a, b);" => "in.(a, Ref(b))",
            "y = intersect(a, b);" => "sort(intersect(a, b))",
            "y = union(a, b);" => "sort(union(a, b))",
            "y = setdiff(a, b);" => "sort(setdiff(a, b))",
            "y = fft(x);" => "fft(x)",
            "y = ifft(x);" => "ifft(x)",
            "y = fftshift(x);" => "fftshift(x)",
            "y = ifftshift(x);" => "ifftshift(x)",
            "y = conv2(a, b);" => "DSP.conv(a, b)",
            "y = freqz(b, a);" => "DSP.freqz(b, a)",
            "y = xcorr(a, b);" => "DSP.xcorr(a, b)",
            "y = nonzeros(A);" => "SparseArrays.nonzeros(A)",
            "y = full(S);" => "Matrix(S)",
            "y = spy(S);" => "Plots.spy(S)",
            "y = ss(a, b, c, d);" => "ControlSystems.ss(a, b, c, d)",
            "y = impulse(s);" => "ControlSystems.impulse(s)",
            "y = nyquist(s);" => "ControlSystems.nyquist(s)",
            "y = lsim(s, u, t);" => "ControlSystems.lsim(s, u, t)",
            "y = c2d(s, ts);" => "ControlSystems.c2d(s, ts)",
            "y = feedback(a, b);" => "ControlSystems.feedback(a, b)",
            "y = dcgain(s);" => "ControlSystems.dcgain(s)",
            "y = ifft2(x);" => "ifft(x)",
            "y = im2double(im);" => "Float64.(im)",
            "y = imrotate(im, 30);" => "ImageTransformations.imrotate(im, 30)",
            "y = imfilter(im, h);" => "ImageFiltering.imfilter(im, h)",
            "y = imagesc(A);" => "Plots.heatmap(A)",
            "y = isequaln(a, b);" => "isequal(a, b)",
            "y = typecast(x, 'uint8');" => "reinterpret(UInt8, x)",
            "y = islogical(x);" => "eltype(x) <: Bool",
            "y = isstruct(x);" => "x isa NamedTuple",
            "y = iscell(x);" => "x isa Vector{Any}",
            "y = arrayfun(@f, a);" => "map(f, a)",
            "y = deblank(s);" => "rstrip(s)",
            "y = fileread('f');" => "read(\"f\", String)",
            "y = num2str(x);" => "string(x)",
            "y = str2double(s);" => "parse(Float64, s)",
            "y = strcat(a, b);" => "string(a, b)",
            "y = strtrim(s);" => "strip(s)",
            "y = seqcomplement(s);" => "BioSequences.complement(s)",
            "y = nt2aa(s);" => "BioSequences.translate(s)",
            "y = randseq(n);" => "BioSequences.randdnaseq(n)",
        ]
        for (m, sub) in cases
            @test occursin(sub, convert_matlab(m * "\n"; wrap_script = false).julia)
        end
    end
...
@testset "Mjolnir — Octave classdef differential oracle" begin
    if !octave_available()
        ...
        for (classes, driver, vars) in ORACLE_CLASS_CASES
            ok, info = oracle_check_class(classes, driver, vars)
...
@testset "Mjolnir — Octave differential oracle (Stage 2 correctness)" begin
    if !octave_available()
        ...
        # Cases are shared with the MATLAB CI harness (dev/oracle_export.jl + oracle_compare.jl).
        for (name, mlab, vars) in ORACLE_CASES
            @testset "$name" begin
                @test !convert_matlab(mlab).has_error
                ok, info = oracle_check(mlab, vars)
                ok ||
                    @info "oracle mismatch" name julia = info.julia octave = info.octave jl =
                    info.jlvals mismatched = info.mismatched
                @test ok
            end
        end
    end
