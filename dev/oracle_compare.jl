...
all_cases = Tuple{String, String, Vector{String}, String}[]

for (idx, (name, mlab_src, vars)) in enumerate(ORACLE_CASES)
    id = make_id(name, idx)
    push!(all_cases, (id, mlab_src, vars, ""))
end
...
for (idx, (classes, driver, vars)) in enumerate(ORACLE_CLASS_CASES)
    ...
    push!(all_cases, (id, driver, vars, prelude))
end
...
println()
println("oracle_compare: $(n_compared) cases compared, $(n_agreed) agreed, $(n_diverged) diverged, $(n_skipped) skipped")
